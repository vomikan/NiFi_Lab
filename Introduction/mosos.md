...
### 3.1 Скачивание и установка NiFi
...
#### 1. Перейдите в домашнюю директорию и скачайте дистрибутив
...
#### 2. Установите unzip (если ещё не установлен)
...
#### 5. Создайте символическую ссылку (для независимости от версии)
...
> ✅ Преимущество:  
> При обновлении NiFi — просто измените ссылку:  
> `sudo ln -sf /opt/nifi-2.6.0 /opt/nifi`

#### 6. Создайте пользователя nifi (рекомендуется)
...
#### 7. Назначьте владельца каталога

```bash
sudo chown -R nifi:nifi /opt/nifi-2.5.0
sudo chown -h nifi:nifi /opt/nifi  # для символической ссылки
```

#### 8. Установите NiFi как сервис
...
#### 9. Проверьте, что сервис добавлен
...
### 3.3 Настройка конфигурации NiFi
...
#### 3.3.1 Настройка памяти (Xms / Xmx)
...
#### 3.3.2 Настройка сборщика мусора (Garbage Collector)
...
##### 2. В `nifi-env.sh` задайте переменные окружения:
...
#### 3.3.4 Указание JAVA_HOME

Убедитесь, что `JAVA_HOME` задан **в `nifi-env.sh`**, чтобы NiFi не зависел от глобальной переменной.
...
#### 3.3.5 Настройка уровня логирования
...
#### 3.3.6 Отключение архивации контента (экономия диска)
