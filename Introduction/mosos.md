...
### 2.2.2 Настройка аутентификации: Bootstrap Token

Команда `set-single-user-credentials` **несовместима с TLS и кластерным режимом** и не рекомендуется к использованию в NiFi 2.5.0.
Вместо этого используйте **Bootstrap Token** — безопасный способ первоначального входа в веб-интерфейс.

#### 2.2.2.1. Сгенерируйте надёжный токен
Используйте любой способ генерации случайной строки. Например:
```bash
openssl rand -base64 32
```
Пример вывода:
```
k3J9d8Xm6Q2pL7vN5rT8wA1bC4xZ7yE6aF9nK2hP4sW=
```
> ✅ Сохраните этот токен — он понадобится для входа. 

#### 2.2.2.2. Добавьте токен в bootstrap.conf
Откройте файл:
```bash
...
```conf
# Bootstrap Token для доступа к веб-интерфейсу
java.arg.19=-Dbnifitoken=k3J9d8Xm6Q2pL7vN5rT8wA1bC4xZ7yE6aF9nK2hP4sW=
```
> 🔐 Замените k3J9d8Xm6Q2pL7vN5rT8wA1bC4xZ7yE6aF9nK2hP4sW= на ваш сгенерированный токен. 
> 💡 Номер 19 — просто порядковый номер java.arg. Убедитесь, что он не дублируется. 
...
# Отключить кластер (standalone mode)
...
#### 2.4 Проверка
После настройки TLS и запуска NiFi необходимо убедиться, что сервис корректно стартовал, слушает нужные порты и готов к работе в кластерном режиме.
...
#### 2.4.2. Проверка состояния сервиса
```bash
...
#### 2.4.3. Проверка прослушивания портов
Убедитесь, что NiFi слушает порт 8443 (HTTPS) и 8082 (кластерный протокол Raft):

```bash
ss -tulnp | grep ':8443\|:8082'
```
Ожидаемый вывод:
```
tcp LISTEN 0 50 *:8443 *:* users:(("java",pid=...,fd=...))
tcp LISTEN 0 50 *:8082 *:* users:(("java",pid=...,fd=...))
```
> 🔍 Порт 8082 — используется для внутреннего взаимодействия между нодами кластера.

#### 2.4.4. Проверка ответа веб-интерфейса (через curl)

Проверьте, отвечает ли веб-сервер. Выполните команду на самой ноде:

```bash
curl https://$(uname -n):8443/nifi/ -s --insecure | head -10
```
...
#### 2.4.5. Проверка через веб-браузер
...
> 💡 **Пример:** https://nifi-node-1:8443/nifi/

**При первом запуске:**
> 💡 Добавьте исключение для самоподписанного сертификата (поскольку используется TLS с self-signed CA).
> ✅ После успешного входа откроется главный интерфейс NiFi — рабочая область для создания и управления сценариями.

#### 2.4.6. Проверка логов
...
#### 2.4.7. Дополнительные полезные команды
```bash
...
После завершения настройки запустите сервис:
