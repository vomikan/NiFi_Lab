...
#### 1.2.3 создание пользователя для NiFi (сервисный аккаунт)
Для безопасной работы NiFi требуется специальный системный пользователь без прав входа в систему.

##### 1.2.3.1. Создание пользователя:

```bash
sudo useradd -r -d /opt/nifi -s /sbin/nologin nifi
```
Параметры команды:
...
##### 1.2.3.2. Настройка прав доступа:

```bash
sudo chown -R nifi:nifi /opt/nifi-2.5.0/
sudo chmod 750 /opt/nifi-2.5.0/
```

##### 1.2.3.3. Проверка создания пользователя:
...
##### 1.2.3.4. Настройка лимитов (в /etc/security/limits.conf):
...
Проверка работы:
```bash
sudo -u nifi whoami  # должен вернуть "nifi"
sudo -u nifi bash    # должен отказать с сообщением об ошибке
```

#### 1.2.3 Проверка настроек системы
Для сервисного пользователя nifi проверка лимитов выполняется без перелогина:

```bash
# Проверка лимитов для пользователя nifi
sudo -u nifi bash -c "ulimit -n"    # Должно быть 50000
sudo -u nifi bash -c "ulimit -u"    # Должно быть 10000
```

# Альтернативный способ (если nologin shell)
```
sudo prlimit --pid $(pgrep -u nifi java) --nofile --nproc
```
...
# Ожидаемый вывод: net.ipv4.ip_local_port_range = 32768    60999

# Проверка swappiness
sudo /sbin/sysctl vm.swappiness
# Ожидаемое значение: 1-10 (для серверов)

# Проверка всех параметров, связанных с NiFi
sudo /sbin/sysctl -a | grep -E 'vm.max_map_count|fs.file-max'
```
...
#### 3.1.1. Перейдите в домашнюю директорию и скачайте дистрибутив
...
#### 2.1.4. Создайте пользователя nifi (рекомендуется)
...
> `-s /bin/false` — запрет входа

#### 2.1.5 Создайте группу nifi
sudo groupadd nifi

#### 2.1.6. Назначьте владельца каталога

```bash
sudo chown -R nifi:nifi /opt/nifi-2.5.0
```

#### 2.1.7. Установите NiFi как сервис
...
RestartSec=10s
LimitNOFILE=50000
PIDFile=/opt/nifi-2.5.0/run/nifi.pid
...
> ✅ После установки сервис будет доступен как:
...
- Сборщик мусора (GC)
- Кодировку (для поддержки русских символов)
- Уровень логирования
- Архивацию контента
...
#### 2.2.1 Настройка памяти (Xms / Xmx)
...
#### 2.2.2 Настройка сборщика мусора (Garbage Collector)
...
##### 2.2.3.2. В `nifi-env.sh` задайте переменные окружения:
...
#### 2.2.3.4 Настройка уровня логирования
...
#### 2.2.3.5 Отключение архивации контента (экономия диска)
