...
### Оптимизация JVM (обязательно для стабильной работы под нагрузкой)

NiFi 2.6.0 + Java 21 по умолчанию на Windows использует G1GC, который может давать паузы до нескольких секунд.  
Для нулевых пауз и максимальной производительности включите ZGC — это стандарт 2025 года.

Откройте файл `conf\bootstrap.conf` и замените/добавьте следующие строки (комменты по-русски не добавляйте):

```properties
# Размер кучи (подберите под свою машину: минимум 4 ГБ, лучше 8–16 ГБ)
java.arg.2=-Xms4g
java.arg.3=-Xmx4g

# Включаем ZGC и две критически важные опции
java.arg.10=-XX:+UseZGC
java.arg.11=-XX:+AlwaysPreTouch
java.arg.12=-XX:+DisableExplicitGC
```
